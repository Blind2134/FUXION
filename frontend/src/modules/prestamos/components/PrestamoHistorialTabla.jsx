...
import { Printer, Filter, X, UserCheck, UserMinus } from "lucide-react";

export const PrestamoHistorialTabla = ({ prestamos, onMarcarPagado }) => {
  const [filtroDeudor, setFiltroDeudor] = useState("");
  const [filtroAcreedor, setFiltroAcreedor] = useState("");
  const [filtroEstado, setFiltroEstado] = useState("TODOS");

  // ========================================================================
  // OBTENER LISTA ÚNICA DE SOCIOS
  // ========================================================================
  const { listaDeudores, listaAcreedores } = useMemo(() => {
    if (!prestamos) return { listaDeudores: [], listaAcreedores: [] };

    const deudores = new Set();
    const acreedores = new Set();

    prestamos.forEach((p) => {
      deudores.add(p.nombreSocioDeudor);
      acreedores.add(p.nombreSocioAcreedor);
    });

    return {
      listaDeudores: Array.from(deudores).sort(),
      listaAcreedores: Array.from(acreedores).sort(),
    };
  }, [prestamos]);
  ...
  const prestamosFiltrados = useMemo(() => {
    ...
    return prestamos.filter((p) => {
      ...
      const cumpleDeudor =
        !filtroDeudor ||
        p.nombreSocioDeudor.toLowerCase().includes(filtroDeudor.toLowerCase());

      // Filtro por acreedor
      const cumpleAcreedor =
        !filtroAcreedor ||
        p.nombreSocioAcreedor
          .toLowerCase()
          .includes(filtroAcreedor.toLowerCase());

      // Filtro por estado
      const cumpleEstado =
        filtroEstado === "TODOS" || p.estado === filtroEstado;

      return cumpleDeudor && cumpleAcreedor && cumpleEstado;
    });
  }, [prestamos, filtroDeudor, filtroAcreedor, filtroEstado]);
  ...
  const handleGenerarPDF = () => {
    ...
    const contenidoHTML = `
      ...
      <body>
        ...
        ${
          filtroDeudor || filtroAcreedor || filtroEstado !== "TODOS"
            ? `
          <div class="filtros-info">
            <strong>Filtros aplicados:</strong>
            ${filtroDeudor ? `Deudor: ${filtroDeudor}` : ""}
            ${
              (filtroDeudor && filtroAcreedor) ||
              (filtroDeudor && filtroEstado !== "TODOS")
                ? " | "
                : ""
            }
            ${filtroAcreedor ? `Acreedor: ${filtroAcreedor}` : ""}
            ${filtroAcreedor && filtroEstado !== "TODOS" ? " | " : ""}
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex flex-wrap gap-4 items-center justify-between">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            {/* Filtro por Deudor (quien recibe) */}
            <div className="flex items-center gap-2">
              <UserCheck className="text-green-500" size={18} />
              <input
                type="text"
                placeholder="Buscar quien recibe..."
                value={filtroDeudor}
            ...
            {/* Filtro por Acreedor (quien presta) */}
            <div className="flex items-center gap-2">
              <UserMinus className="text-blue-500" size={18} />
              <input
                type="text"
                placeholder="Buscar quien presta..."
                value={filtroAcreedor}
                onChange={(e) => setFiltroAcreedor(e.target.value)}
                className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-48"
              />
              {filtroAcreedor && (
                <button
                  onClick={() => setFiltroAcreedor("")}
                  className="text-gray-400 hover:text-gray-600"
                >
                  <X size={16} />
                </button>
